...
class exkp(nn.Module):
  def __init__(self, n, nstack, dims, modules, num_classes=80, cnv_dim=256):
    ...
    self.ct_cnvs = nn.ModuleList([pool_cross(cnv_dim) for _ in range(nstack)])
    ...
    self.ct_heats = nn.ModuleList([make_kp_layer(cnv_dim, curr_dim, num_classes) for _ in range(nstack)])
    ...
    for hmap_tl, hmap_br, hmap_ct in zip(self.tl_heats, self.br_heats, self.ct_heats):
    ...
    self.ct_regrs = nn.ModuleList([make_kp_layer(cnv_dim, curr_dim, 2) for _ in range(nstack)])
  ...
  def forward(self, inputs):
    ...
    for ind in range(self.nstack):
      ...
      if self.training or ind == self.nstack - 1:
        ...
        hmap_ct = self.ct_heats[ind](cnv_ct)
        regs_ct = self.ct_regrs[ind](cnv_ct)
...
def small_hourglass():
  return exkp(n=5, nstack=1, dims=[256, 256, 384, 384, 384, 512], modules=[2, 2, 2, 2, 2, 4])
...
if __name__ == '__main__':
  ...
  net = nn.DataParallel(small_hourglass().cuda())
  net.load_state_dict(torch.load('ckpt/small/checkpoint.t7'))
